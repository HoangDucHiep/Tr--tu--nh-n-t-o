...
export class PuzzleSolver {
  ...
  // still kind of BFS, but use pq so always expand the node with the lowest heuristic value first
  findSolution() {
    if (!this.isSolvable()) return null;

    const queue = new PriorityQueue((a, b) => a.heuristic - b.heuristic);
    const startNode = new PuzzleNode(this.initialState, this.startX, this.startY);
    queue.push({ node: startNode, heuristic: this.h2(this.initialState, this.goalState) + this.h1(this.initialState, this.goalState) });
    ...
    while (!queue.isEmpty()) {
      ...
      Object.keys(PuzzleNode.MOVES).forEach((move) => {
        ...
        if (nextNode && !visited.has(nextNode.stateKey)) {
          queue.push({ node: nextNode, heuristic: this.h2(nextNode.matrix, this.goalState) + this.h1(nextNode.matrix, this.goalState) }); // push the next node to the queue
        }
      });
    }
